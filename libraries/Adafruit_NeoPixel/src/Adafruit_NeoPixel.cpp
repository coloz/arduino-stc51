/* STC port changes, 2026-09-16: MCS251 sender integration, checked allocation
 * with a prefetch sentinel, pin/rate validation and interrupt-state handling.
 * Based on upstream 1.15.5; see ../README-STC.md and ../upstream.json.
 */
/*!
...
bool Adafruit_NeoPixel::begin(void) {
  if (!stc_neopixel_supported(pin, is800KHz)) {
    begun = false;
    return false;
  }
  if (pin >= 0) {
...
void Adafruit_NeoPixel::updateLength(uint16_t n) {
  ...
  const uint32_t requested = (uint32_t)n * ((wOffset == rOffset) ? 3U : 4U);
  if (requested > 65535UL) {
    pixels = NULL;
    numLEDs = numBytes = 0;
    return;
  }
  numBytes = (uint16_t)requested;
  // The STC sender prefetches one byte while emitting the final data bit.
  if ((pixels = (uint8_t *)malloc((size_t)numBytes + 1U))) {
    memset(pixels, 0, (size_t)numBytes + 1U);
    numLEDs = n;
  } else {
    numLEDs = numBytes = 0;
  }
...
#if !(defined(NRF52) || defined(NRF52_SERIES) || defined(ESP32) || defined(ARDUINO_ARCH_MCS251))
...
//----
  ...
  if (is800KHz) {
#endif
    uint32_t top = (F_CPU / 800000);       // 1.25碌s
    uint32_t t0 = top - (F_CPU / 2500000); // 0.4碌s
    uint32_t t1 = top - (F_CPU / 1250000); // 0.8碌s
    ...
    uint32_t top = (F_CPU / 400000);       // 2.5碌s
    uint32_t t0 = top - (F_CPU / 2000000); // 0.5碌s
    uint32_t t1 = top - (F_CPU / 833333);  // 1.2碌s
    SysTick->LOAD = top - 1; // Config SysTick for NeoPixel bit freq
    SysTick->VAL = 0;        // Set to start value
    for (;;) {
      LL_GPIO_SetOutputPin(gpioPort, gpioPin);
...
#elif defined(ARDUINO_ARCH_MCS251)
  stc_neopixel_show(pixels, numBytes, pin, is800KHz);
#else
