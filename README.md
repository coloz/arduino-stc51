...
当前版本供开发和有限场景验证使用，尚未完成统一正式发布及实板验收。产品板需验证实际时钟、接线和外设功能；编译成功不代表实板验收通过。

本次生产发布仅面向 **Windows 和 macOS**。Windows 的 C++ 流程仍需 WSL；Linux 独立宿主不纳入发布适配和验收。ARM64 Mac 的工具要求见 [C++ 驱动说明](tools/cpp-cli/README.md)。
