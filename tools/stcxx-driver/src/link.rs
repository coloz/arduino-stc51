...
fn select(
    ...
    ensure!(!direct.is_empty(), "missing direct C++ input");
    ...
    for iteration in 0..=candidates.len() {
        ...
        for (symbol, owners) in ambiguous {
            ...
            // Require an identical weak ODR body in every possible provider.
            ...
            for i in owners {
                ...
                let body = pattern
                    ...
                    .as_str();
                let body = attributes.replace_all(body, "");
                identities.insert(body.into_owned());
            }
            ensure!(
                identities.len() == 1,
...
pub fn pipeline(d: &Driver) -> Result<()> {
    ...
    remove(output)?;
    let work = output.parent().unwrap().join("stcxx");
    fs::create_dir_all(&work)?;
...
fn pipeline_inner(d: &Driver, work: &Path, output: &Path) -> Result<()> {
    ...
    for arg in &d.arguments {
        arguments.push(if arg.ends_with(".a") {
    ...
    .collect();
        ...
        roots.is_subset(&defs),
        "missing required Arduino runtime roots"
    );
    roots.extend(
        native_refs
    ...
    d.call(
        ...
        &[
            ...
            "-passes=internalize,deadargelim,globaldce".into(),
            format!(
    ...
    eprintln!("STCXX_NATIVE_LINK=PASS");
    Ok(())
}
