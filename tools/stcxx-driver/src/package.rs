...
pub fn platform(source: &Path, output: &Path) -> Result<()> {
    ...
    write(
        driver.join("VALIDATION.md"),
        fs::read(source.join("tools/stcxx-driver/VALIDATION.md"))?,
    )?;
    copy_tree(
    ...
    let version = text(source.join("platform.txt"))?
        ...
        .to_owned();
    archive(&stage, output, &format!("arduino-stc51-{version}"))
}
pub fn stage_toolchain(source: &Path, destination: &Path) -> Result<()> {
    ...
    write_json(
        ...
        &json!({"schema_version":2,"name":"stcxx-toolchain","version":"0.2.0","host":if cfg!(windows){"windows-x86_64"}else{"darwin-arm64"},"execution":"native","runtime_interpreters":[]}),
    )?;
